...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var auxResult = function(hash, aux) {
                if(!hash) return;
                CheckBlockAccepted(hash, util.reverseHex(shareData.txHash), _this.auxes[aux].daemon, function(accepted, tx, height, value) {
            ...
            */
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, util.reverseHex(shareData.txHash), _this.daemon, function(isAccepted, tx, height, value){
    ...
    function GetBlockTemplate(callback, force){
        _this.daemon.cmd('getblocktemplate',
            [{'mode':'template',"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ]}],
            function(result){
                if (result.error){
    ...
    function CheckBlockAccepted(blockHash,tx, daemon, callback){
		//setTimeout(function(){
			...
			if (validResults.length >= 1) {
				...
				daemon.cmd('gettransaction',
                [tx],
                function (results) {
					if (results[0] && results[0].response) {
						
						var hash = results[0].response.blockhash;
                ...
                function (results) {
							...
							if (results[0] && results[0].response && results[0].response.proofhash == blockHash) {
								callback(true, results[0].response.tx[0], results[0].response.height);
							}
							else {
								callback(false);
							}
